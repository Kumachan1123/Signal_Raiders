...
#include "StepTimer.h"
#include "Game/Scene/SettingScene/SettingUI/SettingUI.h"
...
private:
	...
	std::vector<std::unique_ptr<SettingUI>> m_pUI;// UI
	std::vector<std::unique_ptr<SettingUI>> m_pSelect;// 選択

	const wchar_t* m_pSelectTexturePath;// 選択テクスチャパス

	std::unique_ptr<SettingUI> m_pSelectWindow;// 選択ウィンドウ
