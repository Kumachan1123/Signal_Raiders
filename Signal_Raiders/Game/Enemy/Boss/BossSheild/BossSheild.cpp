...
void BossSheild::Update(float elapsedTime)
{
	if (m_isSheild)// シールドが展開されている間
	{
		...
		DirectX::BoundingSphere sphere = m_pBoss->GetBoundingSphere();// ボスの境界球を取得
		sphere.Radius = EnemyParameters::BOSS_SHIELD_RADIUS;// ボスの境界球をシールドの大きさに合わせる
	...
	if (m_sheildHP <= 0)// シールドが破壊されたら
	{
		...
		DirectX::BoundingSphere sphere = m_pBoss->GetBoundingSphere();// ボスの境界球を取得
		sphere.Radius = EnemyParameters::NORMAL_BOSS_RADIUS;// ボスの境界球を元に戻す
