/*
*	@file	CowardlyEnemy.h
*	@brief	すばしっこくて卑怯な敵クラス
*/
#pragma once
#ifndef COWARDLY_ENEMY_DEFINED
#define COWARDLY_ENEMY_DEFINED
// 標準ライブラリ
#include <cassert>
#include <random>
#include <memory>
...
#include "Game/Enemy/CowardlyEnemyModel/CowardlyEnemyModel.h"
...
class EnemyHPBar;
class EnemyBullet;
class CowardlyEnemyModel;
class EnemyManager;
class FPS_Camera;

// すばしっこくて卑怯な敵クラス
class CowardlyEnemy : public IEnemy
{
public:
	// アクセサ
	// 敵の当たり判定を取得する
...
public:
	//	public関数
	// コンストラクタ
	CowardlyEnemy(Player* pPlayer, CommonResources* resources, int hp);
	// デストラクタ
	~CowardlyEnemy();
	// 初期化
...
private:
	...
	// 敵のモデル
	std::unique_ptr<CowardlyEnemyModel>		m_pEnemyModel;
	// 敵のAI
