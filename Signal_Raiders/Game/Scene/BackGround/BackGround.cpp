...
void  BackGround::LoadTexture(const wchar_t* path)
{
	...
	m_texture.push_back(texture);// テクスチャを読み込む
}
/*
*	@brief 生成
...
{
	m_pCreateShader->CreateVertexShader(L"Resources/Shaders/CRT/VS_CRT.cso", m_vertexShader);// 頂点シェーダーの作成
	m_pCreateShader->CreatePixelShader(L"Resources/Shaders/CRT/PS_CRT.cso", m_pixelShader);// ピクセルシェーダーの作成
