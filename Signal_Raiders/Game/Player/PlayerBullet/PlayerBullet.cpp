...
PlayerBullet::~PlayerBullet()
{
	...
	m_pixelShader.Reset();// ピクセルシェーダーの解放
	m_bulletTrail.reset();// 軌跡ポインターの解放
	m_commonResources = nullptr;// 共通リソースの解放
}
/*
*	@brief	初期化
...
void PlayerBullet::Initialize(CommonResources* resources)
{
	...
	m_commonResources = resources;// 共通リソースの設定
	auto device = m_commonResources->GetDeviceResources()->GetD3DDevice();// デバイスの取得
	DrawCollision::Initialize(m_commonResources);// 当たり判定可視化用クラスの初期化
	m_bulletTrail = std::make_unique<Particle>(ParticleUtility::Type::PLAYERTRAIL, BulletParameters::PLAYER_BULLET_SIZE);// 弾の軌道生成
	m_bulletTrail->Initialize(m_commonResources);// 弾の軌道生成の初期化
	...
	m_model->UpdateEffects([&](DirectX::IEffect* effect)// エフェクトの更新
		{
			auto basicEffect = dynamic_cast<DirectX::BasicEffect*>(effect);// ベイシックエフェクトを取得する
			basicEffect->SetDiffuseColor(DirectX::Colors::SkyBlue);// ディフューズカラーを設定する
			basicEffect->SetEmissiveColor(DirectX::Colors::Cyan);// エミッションカラーを設定する
		});
...
{
	auto context = m_commonResources->GetDeviceResources()->GetD3DDeviceContext();// デバイスコンテキストの取得
	auto states = m_commonResources->GetCommonStates();// 共通ステートの取得
	m_bulletTrail->CreateBillboard(m_cameraTarget, m_cameraEye, m_cameraUp);// 軌跡のビルボード行列を作成
	m_bulletTrail->Render(view, proj);// 軌跡描画
	m_model->Draw(context, *states, m_worldMatrix, view, proj);// 弾描画
}
/*
*	@brief	影の描画
...
void PlayerBullet::RenderShadow(DirectX::SimpleMath::Matrix view, DirectX::SimpleMath::Matrix proj)
{
	...
	m_model->Draw(context, *states, shadowMatrix, view, proj, true, [&]()// 影描画
		{
			...
			context->PSSetShader(m_pixelShader.Get(), nullptr, 0);// ピクセルシェーダーの設定
		});
}
/*
*	@brief	当たり判定の描画
